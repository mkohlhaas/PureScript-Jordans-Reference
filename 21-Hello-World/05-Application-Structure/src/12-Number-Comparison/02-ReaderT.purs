...
program
  ∷ ∀ m
  . -- for any monad supporting these capabilities/effects--
  LogToScreen m
  ⇒ --   one of which is logging a message
  GenerateRandomInt m
  ⇒ --   one of which is generating a random int
  MonadAsk Environment m
  ⇒ --   one of which is getting info from a global record/config
...
program = do
  ...
  let
    comparisonResult = case compare hardInt randomInt of
      LT → " < "
      GT → " > "
      EQ → " = "
