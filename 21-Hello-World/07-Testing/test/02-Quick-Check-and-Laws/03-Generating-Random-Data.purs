...
main =
  ...
  printData "explanation" $
    ...
    printData
      ( "elements - Choose an random element from the array where \
    ...
    printData
      ( "shuffle - randomize the order of an array's elements \
        \(e.g. " <> show array <> ")"
      ) $
      shuffle array
    ...
    printData
      ( "oneOf - Randomly choose a generator (where each generator has \
    ...
    let
      array_1 = NEA.singleton 1 -- (NonEmptyArray Int)
      ...
      moreOften = 4.0
    printData
      ( "frequency - Generate an instance from an array of generators \
    ...
    printData
      "suchThat - Create a generator (e.g. even numbers) by filtering \
    ...
    printData
      "arrayOf - Using a generator that creates one `a`, create \
    ...
    printData
      "arrayOf1 - Using a generator that creates one `a`, create \
      \an non-empty array of randomly-generated `a` instances"
      $ arrayOf1
      $ chooseInt 0 9

    printData
      "vectorOf - Using a generator that creates one `a`, create \
    ...
    printData
      "listOf - Using a generator that creates one `a`, create \
