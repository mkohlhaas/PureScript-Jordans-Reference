...
instance Functor Box where
  map ∷ ∀ a b. (a → b) → Box a → Box b
  map f (Box a) = Box (f a)

-- infixl 4 map as <$>

instance Apply Box where
  apply ∷ ∀ a b. Box (a → b) → Box a → Box b
...
------------------------------
pure_no_sugar ∷ ∀ a b. (a → b) → a → Box b
pure_no_sugar f a = pure (f a)

pure_sugar ∷ ∀ a b. (a → b) → a → Box b
pure_sugar f a = ado
  in f a

------------------------------
map_no_sugar ∷ ∀ a b. (a → b) → Box a → Box b
map_no_sugar f g = (\x → f x) <$> g

map_sugar ∷ ∀ a b. (a → b) → Box a → Box b
map_sugar f g = ado
  x ← g
  in f x
...
liftN_no_sugar ∷ ∀ a b c. (a → b → c) → Box a → Box b → Box c
liftN_no_sugar f g h = (\x y → f x y) <$> g <*> h

liftN_sugar ∷ ∀ a b c. (a → b → c) → Box a → Box b → Box c
...
------------------------------
liftN_unit_no_sugar ∷ ∀ a b. (a → b) → Box a → Box Unit → Box b
liftN_unit_no_sugar f g h = (\x _ → f x) <$> g <*> h

liftN_unit_sugar ∷ ∀ a b. (a → b) → Box a → Box Unit → Box b
...
------------------------------
liftN_Let_no_sugar ∷ ∀ a. (Int → Int → a) → Box Int → Box Unit → Box a
liftN_Let_no_sugar f g h = (\x → let y = x + 1 in (\_ → f x y)) <$> g <*> h

liftN_Let_sugar ∷ ∀ a. (Int → Int → a) → Box Int → Box Unit → Box a
